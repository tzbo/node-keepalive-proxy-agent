...
class myAgent extends https.Agent {
    ...
    createConnectionHttpsAfterHttp(options, cb) {
        ...
        if (options.proxy.protocol === 'https:') {
            proxySocket = tls.connect(+options.proxy.port, options.proxy.hostname || options.proxy.host, { rejectUnauthorized: false })
        } else {
            proxySocket = net.connect(+options.proxy.port, options.proxy.hostname || options.proxy.host)
        }
